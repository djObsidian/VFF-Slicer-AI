...
**Купола над отверстиями (`--growth-source geodesic`, ⚠️ не рекомендуется).**
3D-карта строится на **локальных BFS-векторах роста** — над отверстием
(например потолок выреза) они смотрят ~вертикально, поэтому потолок
...
глубины → потолок выгибается сильнее (+0.66 → +1.50 мм). **НО это глобальный
размен, а не точечный**: geodesic поднимает искажение по ВСЕЙ модели
(CoV 0.028 → 0.048) и **портит тонкие фичи — лопасти деформируются хуже**.
Бленд bfs↔geodesic линеен (каждые +0.1 мм купола ≈ +0.005 CoV), бесплатного
купола нет. Для деталей с тонкими элементами держи дефолт `bfs`; точечная
геодезика только у отверстий — открытая задача (см. backlog).
