...
`3d` использует `--max-tilt`, `--smooth-sigma`, `--pitch`, `--extrusion-comp`,
`--subdivide-error`; игнорирует `--dz-per-layer`/`--dz-auto-fit`/`--depth-method`.
Интерактивный вьювер пока показывает только Z-only.
