...
игнорирует `--dz-per-layer`/`--dz-auto-fit`/`--depth-method`. Интерактивный
вьювер пока показывает только Z-only. Статус: прямая карта, обратимость и
компенсация провалидированы на реально нарезанном g-code (98.7% containment);
реальная FDM-печать — следующий шаг. Несходимость на самых кончиках (доли %)
лечится сглаживанием/`--max-tilt` поменьше.
