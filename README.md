...
**Сглаживание (`--smooth-sigma`, по умолчанию 2).** Сырая `Φ` только C0
(трилинейна на вокселях), поэтому плотно нарезанный g-code ловит огранку на
каждой границе вокселя → видимая рябь на поверхности (~0.1 мм при pitch=1), а
резкие повоксельные несогласованности делают карту неинъективной на кончиках
(Ньютон расходится → выбросы). Гауссово сглаживание поля смещений `U` делает
`Φ` ~C1: рябь падает 110→13 µm, а несходимость 5%→0% (карта становится
глобально инъективной). **Должно совпадать между экспортом и inverse** (как
`dz` для Z-only).

**Компенсация экструзии (`--extrusion-comp`, вкл по умолчанию).** Поверхности
роста не равноудалены — деформация локально растягивает/сжимает дорожку.
Слайсер считал `E` для деформированной геометрии, поэтому домножаем на
`1/det(JΦ)` (объёмное отношение, сэмплится по якобиану карты). Средний фактор
≈ `1/vol_ratio`; требует относительной экструзии (M83).

Результаты на пропеллере (pitch=1, smooth-sigma=2) против Z-only:

| | Z-only | 3D |
|---|---|---|
| Сохранение объёма (ratio) | 0.71 | **0.95** |
| Боковое искажение (CoV длин рёбер) | 0.148 | **0.028** (×5 меньше) |
| Поверхностная рябь | — | **13 µm** |
| Обратимость | 1D root-find | Ньютон, **~99.4%** точно |
...
**Купола над отверстиями (`--growth-source geodesic`).** 3D-карта строится на
**локальных BFS-векторах роста** — над отверстием (например потолок
цилиндрического выреза) они смотрят ~вертикально, поэтому потолок
**недодувается**: +0.66 мм там, где геодезический обход дырки требует +2.9 мм.
`--growth-source geodesic` строит направление из **градиента геодезической
глубины** (он знает про обход) → потолок выгибается (+0.66 → +1.50 мм), плоская
коробка остаётся тождеством. Опция (дефолт `bfs`), т.к. полный геодезический
купол геометрически агрессивен (растягивает тонкий материал → искажение и
потеря объёма); `geodesic` стоит лишь CoV 0.028→0.048 глобально. **Должен
совпадать между экспортом и inverse.**
...
- **Рябь на поверхностях.** Возникает на этапе inverse (деформированный меш
  гладкий) и НЕ зависит от pitch — значит это не воксельная огранка.
  Сглаживание смещений её не убирает (хотя чинит склады/искажение). Механизм
  пока не определён; кандидаты — трилинейная C0-выборка по своей природе
  (нужна трикубическая?) либо артефакт поточечной инверсии. Требует отдельного
  расследования.
...
- **Несходимость на самых кончиках** (доли %, после сглаживания ~0) —
  глобальная неинъективность под нависаниями; лечится `--max-tilt` поменьше.
- **Полный купол над вырезами.** `--growth-source geodesic` даёт частичный
  купол (~52% геодезического) без роста искажения. Полный купол требует
  растяжения тонкого материала (конфликт «верность ↔ искажение»). Чистое
  решение — локальная геодезика только у отверстий + адаптивный clamp —
  открытая задача.
